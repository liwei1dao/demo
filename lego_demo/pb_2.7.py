...
def buildProto(pbpath, outpath, pbfile):
    cmdstr = 'protoc --go_out={0} -I{1} {1}/{2}.proto'.format(
        outpath, pbpath, pbfile)
    print(cmdstr)
    os.system(cmdstr)
    file_data = ""
    ...
    with io.open(file, "r", encoding='utf-8') as f:
        for line in f:
            if 'tags:' in line:
                ...
                line = re.sub(
                    r"`(.+?)`", "`{0}`".format(tagsstr[0:len(tagsstr)-1]), line)
            file_data += line
    with io.open(file, "w", encoding='utf-8') as f:
        f.write(file_data)


buildProto('./pb/proto', './pb', 'db_article')
buildProto('./pb/proto', './pb', 'msg_system')
buildProto('./pb/proto', './pb', 'msg_article')
